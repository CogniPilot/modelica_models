...
algorithm
  ...
  for row in 1:size(A, 1) loop
    for column in 1:row loop
      ...
      if row == column then
        // AFFIRMATIVE acceptance: the pivot must be PROVEN to exceed the
        // threshold before it is used. The previous form tested the
        // REJECTION condition (`value <= pivotThreshold`) and fell through
        // to `sqrt(value)` otherwise. Every comparison against a NaN is
        // false, so a NaN pivot -- which is what a non-finite entry in A
        // produces, and A here is the innovation covariance built from the
        // filter covariance and the sensor-reported measurement covariance
        // -- took the "pivot is fine" branch with `ok` still true. The
        // caller then received `ok = true` alongside an all-NaN solve and
        // acted on it. Stating the condition the pivot must satisfy makes
        // NaN and -Inf fail INTO rejection.
        //
        // For every finite `value` the two forms are exact complements, so
        // this is bit-identical on all well-formed inputs; only the
        // non-finite paths change, and only towards rejection.
  ...
  if ok then
    for rhs in 1:size(B, 2) loop
      ...
      for reverseRow in 1:size(A, 1) loop
        ...
        // The real instance was DubinsPolynomial.smoothOffsets, repaired in
        // commit 2b255b3.
