...
TUPLE_ASSIGNMENT = re.compile(r"^\s*\([^()]*,[^()]*\)\s*:=", re.MULTILINE)
...
def check(root: Path) -> None:
    ...
    for relative in (Path("LinearAlgebra/solve.mo"), Path("LinearAlgebra/solveSPD.mo")):
        guarded_file = root / relative
        if not guarded_file.is_file():
            errors.append(f"guarded solver {relative} is missing; update this check")
        elif TUPLE_ASSIGNMENT.search(guarded_file.read_text(encoding="utf-8")):
            errors.append(
                f"{relative} contains a multi-output (tuple-assigning) call; this "
                "combination silently miscompiles under Rumoca and corrupts the "
                "solver (see the guard comment in that file)"
            )
