...
algorithm
  ...
  for candidateIndex in 2:6 loop
    if (candidateIndex <= 4 or allowThreeTurnPaths)
        and candidates[candidateIndex].feasible
        and (not candidates[bestIndex].feasible
          or candidates[candidateIndex].length
            < candidates[bestIndex].length) then
