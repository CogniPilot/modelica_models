...
def plot_missions(
    ...
    axes[0, 0].plot(signal(local, "x_m"), signal(local, "y_m"), label="local mode")
    axes[0, 0].plot(
        signal(glob, "x_m"),
        signal(glob, "y_m"),
        linestyle=":",
    ...
    axes[0, 1].plot(signal(local, "time_s"), signal(local, "z_m"), label="altitude")
    axes[0, 1].plot(
        signal(local, "time_s"),
        signal(local, "target_z_m"),
        linestyle="--",
        label="target",
    )
    ...
    axes[1, 0].plot(
        signal(local, "longitude_deg"),
        signal(local, "latitude_deg"),
        label="global track",
    )
...
def main() -> None:
    ...
    report = {
        ...
        "provenance": {
            "mission": {
                "path": "Vehicles/Rdd2/Qualification/WaypointMission.mo",
