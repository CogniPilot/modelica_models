...
SCENARIO_DIR = ROOT / "Vehicles" / "Cubs2" / "Test"
PATTERN_WAYPOINTS = [
    (0.0, 0.0, 3.0),
    (12.0, 0.0, 3.0),
    (30.0, 0.0, 3.0),
...
def sample(
    ...
    matches = [column for column in columns if column.endswith(f".{name}")]
    if len(matches) == 1:
        return columns[matches[0]][index]
...
def normalize_rumoca_result(result: rum.Result, mode: str) -> list[dict[str, float | str]]:
    ...
    for index in range(count):
        vx = sample(columns, index, "velocity_m_s[1]", default=0.0)
        vy = sample(columns, index, "velocity_m_s[2]", default=0.0)
        heading = math.atan2(vy, vx) if abs(vx) + abs(vy) > 1e-12 else 0.0
        desired_altitude = (
            3.0
            if mode in {"altitude", "heading", "pattern"}
            else sample(columns, index, "position_m[3]")
        )

        rows.append({
            "time": sample(columns, index, "time_s"),
            "mode": mode,
            "x": sample(columns, index, "position_m[1]"),
            "y": sample(columns, index, "position_m[2]"),
            "z": sample(columns, index, "position_m[3]"),
            "roll": sample(columns, index, "euler_rad[1]"),
            "pitch": sample(columns, index, "euler_rad[2]"),
            "yaw": sample(columns, index, "euler_rad[3]"),
            "airspeed": sample(columns, index, "airspeed_m_s"),
            "stick_roll": sample(columns, index, "stickCommand[1]"),
            "stick_pitch": sample(columns, index, "stickCommand[2]"),
            "stick_yaw": sample(columns, index, "stickCommand[3]"),
            "stick_throttle": sample(columns, index, "stickCommand[4]"),
            "surface_ail": sample(columns, index, "actuatorCommand[1]"),
            "surface_elev": sample(columns, index, "actuatorCommand[2]"),
            "surface_rud": sample(columns, index, "actuatorCommand[3]"),
            "surface_thr": sample(columns, index, "actuatorCommand[4]"),
            "airborne": sample(columns, index, "airborne", default=0.0),
            "current_waypoint": sample(columns, index, "currentWaypoint", default=1.0),
            "laps": sample(columns, index, "lapCount", default=0.0),
            "desired_heading": sample(columns, index, "setpoints.heading", default=0.0),
            "desired_altitude": desired_altitude,
            "desired_flight_path_angle": sample(columns, index, "setpoints.flightPathAngle", default=0.0),
            "desired_acceleration": sample(columns, index, "setpoints.acceleration", default=0.0),
            "heading": heading,
            "course_error": sample(columns, index, "courseError_rad", default=0.0),
            "roll_command": sample(columns, index, "rollCommand_rad", default=0.0),
            "inner_roll_command": sample(columns, index, "attitudeCommand_rad[1]", default=0.0),
            "pitch_command": sample(columns, index, "attitudeCommand_rad[2]", default=0.0),
            "tecs_pitch_command": sample(columns, index, "tecsCommands.pitch", default=0.0),
            "tecs_thrust_command": sample(columns, index, "tecsCommands.thrust", default=0.0),
            "mission_phase": sample(columns, index, "missionPhase", default=1.0),
        })
...
def write_markdown_report(stages: dict[str, list[dict[str, float | str]]], check_results: list[tuple[str, str, str]], plot_paths: list[Path]) -> Path:
    ...
    lines = [
        "# CUBS2 open-model scenario verification",
        "",
        "Rumoca compiles and executes each Modelica scenario; Python only normalizes the returned trace for checks and plots.",
        "",
        "These scenarios exercise the deployable outer loop against an explicit simulation surrogate. The real onboard inner loop is proprietary and unavailable, so these results neither establish equivalence to it nor qualify the complete aircraft control system.",
        "",
...
def write_html_report(check_results: list[tuple[str, str, str]], plot_paths: list[Path]) -> Path:
    ...
    path.write_text(
        "\n".join([
            ...
            "<head>",
            "<meta charset=\"utf-8\">",
            "<title>CUBS2 Open-Model Scenario Report</title>",
            "<style>",
            ...
            "</head>",
            "<body>",
            "<h1>CUBS2 Open-Model Scenario Report</h1>",
            "<p>These scenarios exercise the deployable outer loop against an explicit simulation surrogate. The real onboard inner loop is proprietary and unavailable; this report does not establish equivalence to it or qualify the complete aircraft control system.</p>",
            "<h2>Checks</h2>",
...
if __name__ == "__main__":
    try:
        ...
        print(f"open-model scenario assertion failed: {exc}", file=sys.stderr)
        raise SystemExit(1)
